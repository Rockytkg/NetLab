...
// Codec implements Huawei V1 without a packet Authenticator.
type Codec struct{}
...
func (Codec) Decode(raw []byte) (*protocol.Packet, error) {
	...
	p := &protocol.Packet{Version: Version, Type: raw[1], AuthType: raw[2], SerialNo: binary.BigEndian.Uint16(raw[4:6]), RequestID: binary.BigEndian.Uint16(raw[6:8]), UserIP: netip.AddrFrom4([4]byte(raw[8:12])), UserPort: binary.BigEndian.Uint16(raw[12:14]), ErrorCode: raw[14]}
...
func validateAttribute(attr protocol.Attribute) error {
	...
	case protocol.AttrBASIP, protocol.AttrUserPrivateIP:
		if len(attr.Value) != 4 {
			return protocol.ErrInvalidAttribute
		}
...
func decodeAttributes(raw []byte, count int, packet *protocol.Packet) error {
	...
	for i := 0; i < count; i++ {
		...
		n := int(raw[pos+1])
		if n < 2 || pos+n > len(raw) {
			return protocol.ErrInvalidAttribute
		}
		attr := protocol.Attribute{Type: raw[pos], Value: append([]byte(nil), raw[pos+2:pos+n]...)}
		if err := validateAttribute(attr); err != nil {
			return err
		}
...
func encodeAttributes(attrs []protocol.Attribute) ([]byte, error) {
	raw := make([]byte, 0)

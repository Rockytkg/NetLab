...
func (h *UpdateHandler) Handle(acctCtx *accounting.AccountingContext) error {
	...
	err := h.sessionRepo.UpdateCounters(acctCtx.Context, &online)
	if err != nil {
...
func buildOnlineFromRequest(acctCtx *accounting.AccountingContext, vr *vendorparserspkg.VendorRequest) model.RadiusOnline {
	r := acctCtx.Request

...
// na so later filtering and presentation layers can treat absence
// consistently across NAS implementations.
func (h *StartHandler) buildRadiusOnline(r *radius.Request, vr *vendorparserspkg.VendorRequest, nas *model.RadiusNas, nasrip string) model.RadiusOnline {
	...
	return model.RadiusOnline{
		Username:            rfc2865.UserName_GetString(r.Packet),
		NasId:               ifEmptyStr(rfc2865.NASIdentifier_GetString(r.Packet), na),
		NasAddr:             nas.Ipaddr,
		NasPaddr:            nasrip,
		SessionTimeout:      int(rfc2865.SessionTimeout_Get(r.Packet)),
		FramedIpaddr:        ifEmptyStr(rfc2865.FramedIPAddress_Get(r.Packet).String(), na),
		FramedNetmask:       ifEmptyStr(rfc2865.FramedIPNetmask_Get(r.Packet).String(), na),
		...
		MacAddr:             ifEmptyStr(vr.MacAddr, na),
		NasPort:             int64(rfc2865.NASPort_Get(r.Packet)),
		NasClass:            classAttrOrNA(r.Packet),
		NasPortId:           ifEmptyStr(rfc2869.NASPortID_GetString(r.Packet), na),
		NasPortType:         int(rfc2865.NASPortType_Get(r.Packet)),
		ServiceType:         int(rfc2865.ServiceType_Get(r.Packet)),
		...
		AcctOutputPackets:   int64(rfc2866.AcctOutputPackets_Get(r.Packet)),
...
func (h *StartHandler) buildRadiusAccounting(online *model.RadiusOnline, start bool) model.RadiusAccounting {
	accounting := model.RadiusAccounting{
	...
	if !start {
		stopTime := time.Now()
		accounting.AcctStopTime = &stopTime
	}

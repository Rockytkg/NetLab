...
> 一个类 EVE-NG 的网络仿真平台 —— 基于 Web 的虚拟网络拓扑创建、编辑与监控系统。

NetLab 采用 **React 前端 + Go 后端** 架构，提供在线创建、编辑和监控虚拟网络拓扑的能力。
...
- 🖥️ **拓扑编辑**：可视化网络拓扑编辑器（规划中）
- 📊 **实时监控**：设备控制台与实时状态监控（规划中）
...
│   │   ├── pages/               # login/、dashboard/、lab/、settings/ 等页面
│   │   ├── router/              # 单文件路由配置
│   │   ├── stores/              # Zustand 状态管理（appStore、authStore、labStore）
...
| 阶段 | 状态 | 范围 |
|------|------|------|
| Phase 1 | ✅ 已完成 | 布局框架、主题、路由、i18n、认证体系、仪表盘 Lab 列表、占位页面 |
| Phase 2 | 🔲 规划中 | AntV X6 拓扑编辑器、设备面板、设备配置抽屉、Lab 编辑器 |
| Phase 3 | 🔲 规划中 | Lab 监控（G6）、设备控制台（xterm.js + WebSocket）、实时更新 |
| Phase 4 | 🔲 规划中 | 暗色模式、响应式优化、快捷键、协同编辑（Yjs）、WASM |

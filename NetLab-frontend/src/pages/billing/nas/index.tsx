...
export default function NasPage() {
  ...
  const columns: ColumnsType<RadiusNasItem> = [
    ...
    },
    {
      ...
      key: 'vendorCode',
      width: 110,
      render: (val: string) => <Text>{vendorLabel(val)}</Text>,
    },
    {
        ...
        <Form form={form} layout="vertical" requiredMark={false}>
          <Tabs
            ...
            items={[
              {
                key: 'identity',
                label: t('radius:nas.sections.identity'),
                children: (
                  <Row gutter={[16, 0]}>
                    <Col xs={24} sm={12}>
                      <Form.Item name="name" label={t('radius:nas.form.name')} rules={[{ required: true, message: t('radius:nas.form.nameRequired') }]}>
                        <Input maxLength={64} />
                      </Form.Item>
                    </Col>
                    <Col xs={24} sm={12}>
                      <Form.Item name="vendorCode" label={t('radius:nas.form.vendorCode')}>
                        <Select showSearch={{ optionFilterProp: 'label' }} options={RADIUS_VENDOR_CODES.map((v) => ({ value: v.value, label: t(v.labelKey) }))} />
                      </Form.Item>
                    </Col>
                    <Col xs={24} sm={12}>
                      <Form.Item name="ipaddr" label={t('radius:nas.form.ipaddr')} rules={[{ required: true, message: t('radius:nas.form.ipaddrRequired') }]}>
                        <Input maxLength={128} />
                      </Form.Item>
                    </Col>
                    <Col xs={24} sm={12}>
                      <Form.Item name="identifier" label={t('radius:nas.form.identifier')} tooltip={t('radius:nas.form.identifierTip')}>
                        <Input maxLength={128} />
                      </Form.Item>
                    </Col>
                  </Row>
              ...
              {
                key: 'connection',
                label: t('radius:nas.sections.connection'),
                children: (
                  <Row gutter={[16, 0]}>
                    <Col xs={24} sm={12}><Form.Item name="hostname" label={t('radius:nas.form.hostname')}><Input maxLength={128} /></Form.Item></Col>
                    <Col xs={24} sm={12}><Form.Item name="model" label={t('radius:nas.form.model')}><Input maxLength={64} /></Form.Item></Col>
                    <Col xs={24} sm={12}><Form.Item name="coaPort" label={t('radius:nas.form.coaPort')}><InputNumber min={1} max={65535} precision={0} style={{ width: '100%' }} /></Form.Item></Col>
                    <Col xs={24} sm={12}><Form.Item name="status" label={t('radius:nas.form.status')}><Select options={[{ value: 'enabled', label: t('radius:common.enabled') }, { value: 'disabled', label: t('radius:common.disabled') }]} /></Form.Item></Col>
                  </Row>
                ),
              },
              {
                key: 'access',
                label: t('radius:nas.sections.access'),
                children: (
                  <Row gutter={[16, 0]}>
                    <Col xs={24}>
                      <Form.Item label={t('radius:nas.form.secret')} required={!editingNas}>
                        <Space.Compact block>
                          <Form.Item name="secret" noStyle rules={[...(editingNas ? [] : [{ required: true, message: t('radius:nas.form.secretRequired') }]), { min: 6, message: t('radius:nas.form.secretMin') }]}>
                            <Input.Password maxLength={128} autoComplete="new-password" placeholder={editingNas ? t('radius:nas.form.secretEditTip') : undefined} />
                          </Form.Item>
                          <Button onClick={handleGenerateSecret}>{t('radius:nas.form.generateSecret')}</Button>
                        </Space.Compact>
                      </Form.Item>
                    </Col>
                    <Col xs={24}><Form.Item name="tags" label={t('radius:nas.form.tags')}><Input maxLength={128} /></Form.Item></Col>
                    <Col xs={24}><Form.Item name="remark" label={t('radius:nas.form.remark')}><Input.TextArea rows={2} maxLength={255} /></Form.Item></Col>
                  </Row>
                ),
              },
            ]}
